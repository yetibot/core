...
(defn record-and-run-expr
  "Top level expression handler. Use this when you want to evaluate a command
   and all the dynamic vars have already been bound.

   At the end of the expression pipeline a :value or :error will be returned.

   Unwrap it and present it formatted for the user."
  [body user yetibot-user & {:keys [parse?]}]
        ...
        (when (and cmd? (not (:yetibot? user)))
          (run!
            (fn [parse-tree]
              (try
                (let [original-command-str (unparse parse-tree)
                      {:keys [value error]} (handle-parsed-expr chat-source
                                                                user
                                                                yetibot-user
                                                                parse-tree)
                      result (or value error)
