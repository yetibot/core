...
(defn history-for-chat-source
  [{:keys [adapter room] :as chat-source}]
...
(defn head
  [{:keys [adapter room] :as chat-source} n]
  (query
    {:where/map
     {:chat-source-adapter (pr-str adapter)
      :chat-source-room room}
...
(defn grep [{:keys [adapter room] :as chat-source} pattern]
  (query
    {:where/clause (str "chat_source_adapter=? AND chat_source_room=?"
                        " AND body ~ ?")
     :where/args  [(pr-str adapter) room pattern]}))
